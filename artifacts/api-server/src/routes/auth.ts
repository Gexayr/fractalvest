...
router.post("/auth/google", async (req, res, next): Promise<void> => {
  try {
    ...
    if (!user) {
      const byEmail = (await db.select().from(usersTable).where(eq(usersTable.email, email)))[0];

      if (byEmail) {
        user = (await db.update(usersTable).set({ googleId }).where(eq(usersTable.id, byEmail.id)).returning())[0];
      } else {
